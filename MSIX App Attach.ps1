...
# Download MSIX App Attach https://download.microsoft.com/download/b/3/0/b3032659-ce0a-4aaa-9909-12a05b569256/f7a301b6f83e49429f506a9efc8c359b.msixbundle
...
} else 
{
	...
	while ($n -gt 0 -and $null -eq $localHostPool) {
		...
		if ($mode -eq "UserLogoff") {
			if ($isAdmin) {
				...
				try {
					...
					if ((Get-AppxPackage| Where-Object {$_.PackageFullName -eq $packageName}).count -gt 0) {
		...
		if ($mode -eq "VmShutdown") {
			...
			try {
				...
				if (Test-Path ($msixJunction+"\"+$packageName)) {
					...
					Remove-Item ($msixJunction+"\"+$packageName) -Force -Verbose -recurse -Confirm:$false
				}
			} catch {
				LogWriter("We got an error: $_")
			}
		...
		foreach ($hostPool in $app.SessionTarget.hostPools) {
			...
			if ($mode -eq "UserLogon" -and $localHostPool -eq $hostPool)
            {
                ...
                foreach ($userGroup in $app.SessionTarget.userGroups) {
                    if (($groups| Where-Object {$_.SamAccountName -eq $userGroup}).count -ne 0) {
                        $isInGroup=$true
                    }
                }
                if ($isInGroup) {
